...
public class SeverBackground {
	...
	private ServerGUI gui; // null // new 하면안됨 !
...
//	private BufferedWriter bw;
	...
	// 서버 생성 및 설정
	public void setting() {
		//참고: 동시접속자로 map 정보가 동기화되어 들어가도록 설정함.
		Collections.synchronizedMap(mapClients);
		try {
			serversocket = new ServerSocket(7777);
			// 방문자 접속을 계속 받아들임.무한반복함. GUI 프로그램경우 창 닫힐떄까지
			// 계속 반복됨. break 없음
			while (true) {
	...
	class Client extends Thread {
		...
		public Client(Socket socket) {
			...
			try {
				...
				// 클라이언트 nickname이 바로 이어서 들어옴.
				//접속이되면 바로 nickname이 전달될 것이므로 읽음.
		...
		public void run() {
			...
			//client와 입력통로가 끊어지지 않았다면 계속 반복확인인함
			//client에서 수신받은 msg
			while(br != null) {
				try {
